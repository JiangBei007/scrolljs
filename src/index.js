		...
		__TouchEnd(timeStamp){
			...
			if (self.__isDragging) {
				...
				if (self.__isSingleTouch && (timeStamp - self.__lastTouchMoveTimeStamp) <= 100) {
					...
					for (let i = endPos; i > 0 && positions[i] > (self.__lastTouchMoveTimeStamp - 100); i -= 2) {
					  startPos = i
					}
					if (startPos !== endPos) {
					  var timeOffset = positions[endPos] - positions[startPos];
					  
					  var movedDistance = self.__scrollPosition - positions[startPos - 1]
